...
Click Create Web Service but don't worry about the first deploy failing — we still need to add a disk and env vars.
3. Add a persistent disk
In the new service, go to Disks → Add Disk.
Fill in:
Name: `data`
Mount path: `/data`
Size: `10 GB` (plenty of headroom for photos; ~$1/mo)
Click Save. The service will restart automatically.
...
Log in as coach.
Click Add Client (or convert a Lead — same flow).
Fill in name, email, start date, and a 4-digit PIN.
...
If the email fails (misconfigured Resend, bad sender domain, etc.), you'll get a dialog telling you why — fall back to texting the PIN manually.
...
In Render: Settings → Custom Domains → Add Custom Domain.
Add a CNAME record at your DNS provider: `coach` → `your-service.onrender.com`.
Wait for SSL to provision (~5 minutes).
